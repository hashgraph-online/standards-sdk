...
describe('HederaMirrorNode', () => {
  ...
  describe('additional endpoints and filters', () => {
    test('accounts memo and airdrops endpoints', async () => {
      ...
      const out = await mirrorNode.getOutstandingTokenAirdrops('0.0.1', {
        limit: 2,
        order: 'desc',
        receiverId: '0.0.2',
        serialNumber: '3',
    ...
    test('contract results/state/logs/actions', async () => {
      ...
      axiosGet.mockResolvedValueOnce({
        data: { state: [{ key: '0x00', value: '0x01' }] },
      });
          ...
          nfts: [
            {
              token_id: '0.0.9',
              serial_number: 1,
              metadata: Buffer.from('ipfs://x').toString('base64'),
            },
          ],
          links: { next: null },
        },
      ...
      messages: [
        {
          ...
          message: Buffer.from(
            JSON.stringify({ p: 'hcs-20', op: 'register' }),
          ).toString('base64'),
          running_hash: 'hash1',
          sequence_number: '1',
          topic_id: '0.0.12345',
        },
        {
          ...
          message: Buffer.from(JSON.stringify({ any: 'json' })).toString(
            'base64',
          ),
          running_hash: 'hash2',
          sequence_number: '2',
  ...
  describe('custom headers and authentication', () => {
    ...
    test('handles URL-based API key replacement', async () => {
      ...
      mirrorWithApiKey.configureRetry({
        maxRetries: 1,
        initialDelayMs: 0,
        maxDelayMs: 0,
        backoffFactor: 1,
