...
describe('Transaction Summary Registry', () => {
  ...
  test('summarizes grouped token transfers', () => {
    ...
    const s = resolveTransactionSummary(tx);
    expect(s).toContain(
      'Transfer of token 0.0.300 from 0.0.1 (5) to 0.0.2 (5)',
    );
    expect(s).toContain(
      'Transfer of token 0.0.400 from 0.0.3 (1) to 0.0.4 (1)',
    );
  });
...
describe('Transaction Summary Registry – extended coverage', () => {
  const base: Partial<
    import('../../src/utils/transaction-parser-types').ParsedTransaction
  > = {
  ...
  test('token freeze/unfreeze/grant/revoke/pause/unpause/wipe/delete', () => {
    expect(
      summarize({
        type: 'TOKENFREEZE',
        tokenFreeze: { tokenId: '0.0.1', accountId: '0.0.2' },
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'TOKENUNFREEZE',
        tokenUnfreeze: { tokenId: '0.0.1', accountId: '0.0.2' },
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'TOKENGRANTKYC',
        tokenGrantKyc: { tokenId: '0.0.3', accountId: '0.0.4' },
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'TOKENREVOKEKYC',
        tokenRevokeKyc: { tokenId: '0.0.3', accountId: '0.0.4' },
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'TOKENWIPE',
        tokenWipeAccount: { tokenId: '0.0.6', accountId: '0.0.7', amount: 10 },
        ...base,
      }),
  ...
  test('token associate/dissociate', () => {
    expect(
      summarize({
        type: 'TOKENASSOCIATE',
        tokenAssociate: { accountId: '0.0.9', tokenIds: ['0.0.1', '0.0.2'] },
        ...base,
      }),
    ...
    expect(
      summarize({
        ...
        tokenDissociate: { accountId: '0.0.9', tokenIds: ['0.0.1'] },
        ...base,
      }),
  ...
  test('account create/update/delete', () => {
    expect(
      summarize({
        type: 'ACCOUNTCREATE',
        cryptoCreateAccount: { initialBalance: '100', alias: '0xabc' } as any,
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'ACCOUNTUPDATE',
        cryptoUpdateAccount: { accountIdToUpdate: '0.0.10' } as any,
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'ACCOUNTDELETE',
        cryptoDelete: { deleteAccountId: '0.0.11' } as any,
        ...base,
      }),
  ...
  test('approve/delete allowance', () => {
    ...
    expect(
      summarize({
        ...
        cryptoDeleteAllowance: { nftAllowancesToRemove: [{}, {}] } as any,
        ...base,
      }),
  ...
  test('contract create/update/delete', () => {
    ...
    expect(
      summarize({
        type: 'CONTRACTUPDATE',
        contractUpdate: { contractIdToUpdate: '0.0.12' } as any,
        ...base,
      }),
    ...
    expect(
      summarize({
        type: 'CONTRACTDELETE',
        contractDelete: {
          contractIdToDelete: '0.0.13',
          transferAccountId: '0.0.14',
        } as any,
        ...base,
      }),
  ...
  test('schedule/system/node/atomic batch summaries', () => {
    expect(
      summarize({
        type: 'SCHEDULECREATE',
        scheduleCreate: { memo: 'payroll' } as any,
        ...base,
      }),
    ).toBe('Create Schedule (Memo: payroll)');
    expect(
      summarize({ type: 'SCHEDULESIGN', scheduleSign: {} as any, ...base }),
    ).toBe('Sign Schedule');
    expect(
      summarize({
        type: 'SYSTEMDELETE',
        systemDelete: { fileId: '0.0.15' } as any,
        ...base,
      }),
    ).toBe('System Delete File 0.0.15');
    expect(
      summarize({
        type: 'SYSTEMUNDELETE',
        systemUndelete: { contractId: '0.0.18' } as any,
        ...base,
      }),
    ).toBe('System Undelete Contract 0.0.18');
    expect(summarize({ type: 'FREEZE', ...base })).toBe('Network Freeze');
    expect(summarize({ type: 'ETHEREUMTRANSACTION', ...base })).toBe(
      'Ethereum Transaction',
    );
    expect(
      summarize({
        type: 'UNCHECKEDSUBMIT',
        uncheckedSubmit: { topicId: '0.0.19' } as any,
        ...base,
      }),
    ).toBe('Unchecked Submit to topic 0.0.19');
    ...
    expect(
      summarize({
        type: 'ATOMICBATCH',
        atomicBatch: { transactions: [{}, {}, {}] } as any,
        ...base,
      }),
    ).toBe('Atomic Batch (3 transaction(s))');
  });
});
