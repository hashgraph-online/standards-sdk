...
jest.mock('@hashgraph/sdk', () => {
  const PrivateKey = {
    fromStringECDSA: (s: string) => ({
      toString: () => s,
      publicKey: { toString: () => 'pk-ecdsa' },
    }),
    fromStringED25519: (s: string) => ({
      toString: () => s,
      publicKey: { toString: () => 'pk-ed25519' },
    }),
  } as any;
  return {
    Client: {
...
jest.mock('../../src/services/mirror-node', () => ({
  HederaMirrorNode: jest.fn().mockImplementation(() => ({
    getTopicInfo: jest.fn(async (topicId: string) => ({
      memo: topicId.endsWith('i') ? 'hcs-2:1:3600' : 'hcs-2:0:3600',
    })),
...
describe('HCS2Client (unit)', () => {
  ...
  test('registerEntry returns sequenceNumber from receipt', async () => {
    ...
    const { buildMessageTx } = jest.requireMock(
