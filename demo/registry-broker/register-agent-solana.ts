...
const resolveSolanaSelection = (): string => {
  const raw = process.env.REGISTRY_BROKER_DEMO_SOLANA_NETWORK?.trim();
...
const main = async (): Promise<void> => {
    ...
    try {
      if (ledgerMode === 'hedera' && hederaLedgerConfig) {
        const verification = await client.authenticateWithLedgerCredentials({
          ...
          label: 'erc-8004-solana registration',
        });
        console.log(
  ...
  const registerOptions: RegisterAgentOptions = {
    ...
    updateAdditionalRegistries: [solanaSelection],
  };

  console.log(
    `Preparing to register agent "${alias}" via ${baseUrl} (mode: ${DEFAULT_MODE}).`,
  );
  console.log(`  Targeting Solana registry: ${solanaSelection}`);
